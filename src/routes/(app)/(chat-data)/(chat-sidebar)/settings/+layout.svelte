...
<div class="flex flex-col h-full place-items-center">
	...
	<div
		class={cn('sticky z-20 top-0 h-0 w-full transition-[height]', {
			'h-14 settings:h-0 [--height:3.5rem] settings:[--height:0] [--content:block] settings:[--content:none]':
	...
	>
		<div class="relative h-(--height) top-0">
			...
			<div class="" style="display: var(--content);">
				{#if scrollState.y > 15}
					<animated.span
	...
	<div class="w-full max-w-3xl flex flex-col py-0 settings:py-4 gap-4 place-items-center px-4">
		{@render children()}
	</div>
</div>
