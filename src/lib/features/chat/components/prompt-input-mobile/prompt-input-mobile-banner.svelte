...
<div
	...
	data-state={dismissed || (dismissedByError && bannerState.rootState.error !== null)
		...
		: 'open'}
	class={cn(
		'ease-in-out border bg-sidebar -translate-y-8 data-[state=closed]:rounded-b-4xl rounded-t-3xl data-[state=open]:h-12.5 absolute z-0 w-full flex items-start justify-between gap-4 px-4 duration-150',
		{
